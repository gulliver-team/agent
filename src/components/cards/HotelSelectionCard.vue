...
<template>
  <div>
      <div class="flex items-center justify-between mb-3">
        ...
        <div class="text-xs text-zinc-600 dark:text-zinc-400">{{ filtered.length }} options</div>
      </div>

      <div v-if="props.data.budget" class="mb-3 flex items-center gap-3">
        <div class="text-sm dark:text-zinc-100">Budget</div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div v-for="h in filtered" :key="h.id" class="border rounded-xl p-3 hover:shadow-soft transition bg-white dark:border-zinc-700 dark:bg-zinc-800">
          <div class="flex items-center justify-between">
            <div>
              <div class="font-medium">{{ h.name }}</div>
              <div class="text-xs text-zinc-600 dark:text-zinc-400">{{ h.address }}</div>
            </div>
            <div class="text-right">
              <div class="font-semibold">${{ h.price }}</div>
              <div class="text-xs text-zinc-600 dark:text-zinc-400" v-if="h.distanceMeters">
